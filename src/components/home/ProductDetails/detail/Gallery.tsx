...
export default function Gallery({ images, alt }: GalleryProps) {
  ...
  return (
    <div className="flex flex-col-reverse md:flex-row gap-3 sm:gap-4">
      {/* Thumbnails */}
      <div className="flex w-full shrink-0 flex-row gap-2 overflow-x-auto py-1 md:py-0 md:w-20 md:max-h-[600px] md:flex-col md:overflow-x-hidden md:overflow-y-auto sm:w-24 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {list.map((src, i) => (
            <button
              key={i}
              type="button"
      ...
      {isFullScreen && (
        <div className="fixed inset-0 z-[100] flex h-screen w-screen items-center justify-center bg-white p-4 sm:p-8">
          ...
          <div className="flex h-full w-full flex-col-reverse md:flex-row items-center gap-4 sm:gap-6 md:gap-12">
            {/* Thumbnails (Modal) */}
